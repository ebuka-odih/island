...
class Controller extends BaseController
{
    ...
    public function autoCreate(){
        $shipment = Product::orderBy('created_at', 'desc')->first();
        if($shipment){
...
//        $shipment_num = (int)$last_shipmemt_num + 1;
    ...
    public function trackingNo(){
        $first_sting = 'IS'; //this is d default starting string
        ...
        $validator = Validator::make(['id'=>$tn],['id'=>'unique:products,shipment_number']);  // ds checks if ds code exist in db
        if($validator->fails()){
